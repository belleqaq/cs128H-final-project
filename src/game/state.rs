...
use crate::game::room::{Decoration, DecorationKind, Door, NpcMarker, Room, RoomStyle};
...
impl GameState {
    ...
    pub fn update(&mut self) {
        ...
        if let Some(target) = self.room().target {
            let interaction_zone = Rect::new(
                target.rect.x - 20.0,
                target.rect.y - 20.0,
                target.rect.w + 40.0,
                target.rect.h + 40.0,
            ...
            if overlaps(player_rect, interaction_zone) {
                self.interaction_hint = Some(format!("Press E to use {}", target.label));
                if is_key_pressed(KeyCode::E) {
                    self.win_flash = 1.2;
                    self.interaction_cooldown = 2.0;
                    self.interaction_message =
                        format!("Interaction complete: you used the {}", target.label);
                }
            }
        }
    }
